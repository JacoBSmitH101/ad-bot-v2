...
export const data = new SlashCommandBuilder()
    .setName("match-reset")
    .setDescription(
        "[ADMIN] Reset a match back to scheduled and delete its result"
    )
    .addStringOption((opt) =>
...
export async function execute(interaction) {
    ...
    try {
        const { season, match } =
            await interaction.client.services.results.adminResetMatch({
        ...
        const embed = new EmbedBuilder()
            .setTitle("🔄 Match reset")
            .setDescription(
                `Match reset to **scheduled** in **${season.name}**`
            )
            .addFields(
